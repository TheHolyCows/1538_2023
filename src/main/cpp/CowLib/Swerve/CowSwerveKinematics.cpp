...
namespace CowLib
{
    ...
    */
    CowSwerveKinematics::CowSwerveKinematics(double wheelBase)
    {
        ...
        for (int i = 0; i < NUM_MODULES; i++)
        {
            ...
            m_ModuleRotations[i] = new frc::Rotation2d(m_ModulePositions[i].X(), m_ModulePositions[i].Y());
            printf("Module %d: %f, %f\n", i, m_ModuleRotations[i]->Degrees().value(), m_ModulePositions[i].X());
        }
        // print mod rotations
        for (int i = 0; i < NUM_MODULES; i++)
        {
    ...
    */
        ...
        const std::array<CowSwerveModuleState, 4> &moduleStates)
    {
        ...
        //     frc::Rotation2d cpy = m_ModuleRotations[i];
        //     //     // try
        //     //     // {
